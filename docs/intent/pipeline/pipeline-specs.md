...
- [x] **PIPE-SCHED-004**: Each workflow shall declare a `concurrency` group with `cancel-in-progress: false`: `ingest-purpleair`, `ingest-airnow`, and `calibrate` for both calibration workflows, so that runs which can write the same partition never overlap.

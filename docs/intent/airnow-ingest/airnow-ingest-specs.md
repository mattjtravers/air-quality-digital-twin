...
- [ ] **AN-API-003**: When fetching a window, the AirNow ingester shall pass `startDate` and `endDate` formatted as `YYYY-MM-DDTHH` in UTC, both inclusive.
...
- [ ] **AN-API-008**: When the fetch window (after the flatline extension of AN-RUN-002) spans more than 24 hours, the AirNow ingester shall split it into consecutive 24-hour requests covering the whole window with no gap and no overlap, and concatenate their rows in window order.
...
- [ ] **AN-MAP-001**: When building a `Site` from a site's validated rows, the AirNow ingester shall set `site_id` per AN-SITE, `source = airnow`, `source_native_id` = `FullAQSCode` as received (falling back to `IntlAQSCode` as received), `site_type = reference_monitor`, `name` from `SiteName`, `latitude`/`longitude` as reported without rounding, and `last_observed_at` = the latest `UTC` among the site's rows in the run.
...
- [ ] **AN-RUN-001**: The AirNow ingester shall expose `ingest_airnow(settings, archive_uri, start, end) -> IngestSummary` as its entry point, which shall fetch the extended window, validate rows, normalize site identifiers, apply QC per site, build records, write sites then observations to the observation store, and return the summary.
- [ ] **AN-RUN-002**: When a run begins, the AirNow ingester shall extend the fetch window backwards by `flatline_hours − 1` hours, fetching `[start − (flatline_hours − 1) h, end]`, so that flatline detection has the hours preceding the requested window.
...
- [ ] **AN-RUN-005**: The AirNow ingester shall require `start` and `end` to be timezone-aware UTC datetimes and shall truncate each to the hour before use; if either is naive, then the run shall fail with a validation error.
- [ ] **AN-RUN-007**: When a run completes, the AirNow ingester shall return an `IngestSummary` with `source = airnow`, `fetched` = total rows across all responses, `rejected` = boundary-rejection reason → count (including `duplicate_site_hour`), `written` = number of observations handed to the store, `flagged` = flag → number of observations carrying it, `partitions` = the URIs the store reported, `window_start`/`window_end` = the requested (unextended) `start`/`end`, and `snapshot_at` unset.

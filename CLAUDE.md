...
Stop after each phase for user review. **Docs carry current intent, written to be read cold** — write each doc as if authored fresh today, from current intent alone: no narration of how it changed, no meaning that needs the conversation that produced it, no rebuttals to questions only a past discussion raised.

**Current and future only — no graveyards.** Rationale and constraints a fresh author would independently write stay, but approaches the project ruled out or dropped do not. Concretely: each LLD's decisions table is `Decision | Chosen | Rationale`, and the Rationale states why the current choice is right rather than why other options were wrong. Do not add an "Alternatives Considered" column, a "Resolved" list of settled questions, or body prose narrating what was tried and abandoned. A property of the world that explains the design ("GitHub Actions' cron trigger drops most occurrences") is current intent and stays; the project's path to discovering it does not. Open questions about *future* work stay.
